...
install_apt_deps() {
  ...
  local packages=("stow" "fzf" "fd-find" "ripgrep" "alacritty" "tmux" "docker" "wget" "tree" "direnv" "zoxide" "npm", "meld")
...
install_go() {
  ...
  if directory_exists /usr/local/go/bin; then
    log_success "Go already installed, skipping..."
    return 0
  fi
